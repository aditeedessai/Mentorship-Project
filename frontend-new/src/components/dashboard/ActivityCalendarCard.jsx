    ...
    >
      ...
      <div className="mb-4 flex items-center justify-between">
        <button
          ...
          className={`rounded-xl p-1.5 transition ${isDarkMode
      ...
      <div className="relative">
        {isLoadingDays && (
          <div
            className={`absolute inset-0 z-10 flex items-center justify-center rounded-2xl backdrop-blur-sm ${isDarkMode ? "bg-[#17131F]/60" : "bg-white/60"
              }`}
          >
        ...
        <div className="grid grid-cols-7 gap-y-1.5 text-center">
          {WEEKDAY_LABELS.map((label) => (
            <span
              key={label}
              className={`text-[10px] font-bold uppercase tracking-wider ${isDarkMode ? "text-white/40" : "text-gray-400"
                }`}
            >
              {label}
            </span>
          ...
          {cells.map((day, idx) => {
            ...
            return (
              <div key={day} className="flex items-center justify-center py-0.5">
                <span
                  className={`flex h-6 w-6 sm:h-7 sm:w-7 items-center justify-center rounded-xl text-[11px] sm:text-xs font-bold transition-all ${isToday
