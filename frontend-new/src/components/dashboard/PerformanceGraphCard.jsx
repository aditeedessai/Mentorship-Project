...
function PerformanceGraphCard({ onNavigate }) {
  ...
  const chartSeries = useMemo(() => {
    if (attempts.length === 0 || availableTypes.length === 0) {
      return [];
    }

    return availableTypes.map((qType) => {
      const config = TYPE_CONFIG[qType] || {
        label: qType,
        color: "#8064C7",
      };
      ...
      attempts.forEach((att, idx) => {
        ...
        if (score !== undefined && score !== null) {
          // X-coordinate based on attempt position
    ...
    >
      ...
      <div className="mb-3.5 sm:mb-4 flex flex-col gap-2.5 sm:gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        {studySets.length > 0 && (
          <div className="flex items-center gap-2">
            <label
              htmlFor="study-set-select"
              className="text-xs font-semibold opacity-60 shrink-0"
            >
              Study Set:
            </label>
            ...
            >
              {studySets.map((set) => (
                <option
                  key={set.study_set_id || set.id}
                  value={set.study_set_id || set.id}
                >
      ...
      {setsLoading || historyLoading ? (
        ...
        <div className="flex h-56 flex-col items-center justify-center gap-3 text-center">
          ...
          >
            Take your first test for this study set to start tracking your
            performance.
          </p>
                  ...
                  <g key={series.qType}>
                    ...
                    {series.points.map((pt, pIdx) => (
                      <circle
                        ...
                        cy={pt.y}
                        r={
                          hoveredPoint?.qType === pt.qType &&
                          hoveredPoint?.attemptNumber ===
                            pt.attemptNumber
                            ? "6"
                            : "4.5"
                        }
              ...
              >
                <div className="flex items-center gap-1.5 font-bold">
                  ...
                  <span className="opacity-60">
                    (Attempt {hoveredPoint.attemptNumber})
                  </span>
                </div>
