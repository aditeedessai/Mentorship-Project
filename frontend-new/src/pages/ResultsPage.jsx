...
// Extracts and cleans the core topic name from the AI hint string
...
export default function ResultsPage({
    ...
    async function loadAllResults() {
      try {
        ...
        if (submittedType) {
          ...
          setActiveSection(
            normalizeTypeName(
              rawList[0].question_type || rawList[0].type
            )
          );
        } else if (perfData?.completed_sections?.length > 0) {
          setActiveSection(
            normalizeTypeName(perfData.completed_sections[0])
          );
        }
      } catch (err) {
  ...
  const totalScore = useMemo(() => {
    if (cumulativeData?.total_marks_obtained !== undefined) {
      return (
        Math.round(
          Number(cumulativeData.total_marks_obtained) * 100
        ) / 100
      );
    }
    return 0;
  ...
  const sectionsList = useMemo(() => {
    ...
    if (evaluations && evaluations.length > 0) {
      ...
      evaluations.forEach((item) => {
        const typeKey = normalizeTypeName(
        ...
        const earned = isSkipped
          ? 0
          : Number(item.marks_awarded ?? item.score ?? 0);
      ...
      return Object.values(grouped).map((g) => ({
        ...
        percentage:
          g.maximumMarks > 0
            ? Math.round((g.marksObtained / g.maximumMarks) * 100)
            : 0,
        remark: null,
      }));
    }
  ...
  const aggregateImprovements = useMemo(() => {
    ...
    const attemptedTypes = new Set(
      aggregateSectionsList.map((s) => s.questionType)
    );
    ...
    if (neverAttempted.length > 0) {
      const formatted = neverAttempted
        .map((t) => SECTION_TITLE_MAP[t] || t)
        .join(', ');
  ...
  // Lookup map for passed questions hints & topics
  ...
  // Active Questions for Section Drawer
  const activeQuestions = useMemo(() => {
    if (!activeSection || !evaluations || evaluations.length === 0) {
      return [];
    }

    const filtered = evaluations.filter((item) => {
      const t = normalizeTypeName(
        item.question_type || item.type
      );
    ...
    return filtered.map((item, idx) => {
      ...
      const awardedMarks = Number(
        item.marks_awarded ?? item.score ?? 0
      );
  ...
  }, [activeSection, evaluations, questionHintMap]);

  // Extract list of all unique weak topics from incorrect questions
  const weakTopics = useMemo(() => {
    ...
    evaluations.forEach((item, idx) => {
      ...
      const isWrong = item.is_correct === false || isSkipped;
      ...
      if (isWrong) {
        ...
        const topic =
          ...
          item.topic ||
          extractTopicFromHint(item.ai_hint) ||
          questionHintMap.get(String(qId)) ||
          questionHintMap.get(`index_${idx}`);
  ...
  const activeWrong = useMemo(() => {
    return activeQuestions.filter((q) => q.isCorrect === false && !q.isSkipped)
      .length;
  }, [activeQuestions]);
    ...
    return (
      ...
      >
        ...
        <div className="relative z-10 flex w-full max-w-xl flex-col items-center px-6 py-12 text-center">
          ...
          >
            Jojo is reviewing your answers and preparing your performance insights.
          </p>
          ...
          >
            <div className="flex items-start gap-3 text-left">
              ...
              >
                Your performance data is being analyzed to identify strengths, weak
                areas, and progress.
              </p>
            </div>
          </div>
          ...
          >
            Please don't close this page while Jojo reviews your performance.
          </p>
        </div>
      </div>
    );
  ...
  if (
    ...
    return (
      <div className="mx-auto mt-20 max-w-md rounded-3xl border border-red-500/30 bg-red-500/10 p-6 text-center text-red-400">
        ...
        <p className="text-base font-black">Failed to load evaluation</p>
  ...
  const completedSectionsCount =
    performanceData?.completed_sections?.length || sectionsList.length;
    ...
    <div className="mx-auto max-w-6xl space-y-6 pb-12 transition-all duration-300">
              ...
              {aggregateSectionsList.map((sec) => (
                ...
                >
                  ...
                  >
                    <Clock size={12} />
                    <span>
                      {sec.attemptsTaken} attempt
                      {sec.attemptsTaken === 1 ? '' : 's'} · last{' '}
                ...
                <div className="space-y-4">
                  {activeQuestions.map((q) => (
                    ...
                    >
                      ...
                      {/* Display Weak Topic Badge on wrong questions */}
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        ...
        >
          ...
          {/* Render Weak Topics identified via AI Hints */}
          {!isAggregateMode && weakTopics.length > 0 && (
            <div className="space-y-2.5 rounded-2xl border border-rose-500/20 bg-rose-500/5 p-3.5">
          ...
          {/* General Section/Progress Recommendations */}
          <ul className="space-y-2">
            ...
            {!isAggregateMode &&
              ...
              improvements.length === 0 && (
                <li className="text-xs text-emerald-400 font-semibold">
                  Excellent work! No weak topics detected in this section.
                </li>
              )}
          </ul>
        </div>
